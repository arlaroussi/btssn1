...
#include<string.h>
    ...
    int main()
    {
        int n,i, nomTaille, prenTaille;
        Personne *pEtud; //On déclare un pointeur sur une structure
        ...
        printf("Donnez la taille max du pr\x82énom:\n");
        scanf("%d",&pTaille);

        for(i=0;i<n;i++) {

            pEtud[i]->nom = (char *) calloc(nomTaille, sizeof(char));
            printf("Entrez le nom  de la personne N%d:\t",i+1);
            scanf("%s",pEtud[i]->nom);
            pEtud[i]->prenom = (char *) calloc(prenomTaille, sizeof(char));
